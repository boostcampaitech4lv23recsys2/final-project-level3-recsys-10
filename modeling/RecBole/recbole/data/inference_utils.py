...
def inference_load_split_dataloaders(config):
...
def inference_data_preparation(config, dataset):
    """Split the dataset by :attr:`config['eval_args']` and create training, validation and test dataloader.

    Note:
        If we can load split dataloaders by :meth:`inference_load_split_dataloaders`, we will not create new split dataloaders.
    ...
    """
    dataloaders = inference_load_split_dataloaders(config)
    if dataloaders is not None:
        ...
        _, _, test_dataset = built_datasets
